...
# 取出分點資料表的兩欄資料，並合併成一欄
df_left = data[['序號','券商','價格','買進股數','賣出股數']]
df_right = data[['序號.1','券商.1','價格.1','買進股數.1','賣出股數']]
df_right = df_right.rename(columns={'券商.1':'券商','價格.1':'價格','序號.1':'序號','買進股數.1':'買進股數','賣出股數':'賣出股數'})
...
temp["均買價"] = combine_df.groupby(by='券商').apply(lambda x: (x['買進股數']*x['價格']).sum() / x['買進股數'].sum())
# 計算均賣價，均賣價為當日賣出總價除以總股數
temp["均賣價"] = combine_df.groupby(by='券商').apply(lambda x: (x['賣出股數']*x['價格']).sum() / x['賣出股數'].sum())
